...
bool is_excellent(int classpoint, int outclass)
{
	if (classpoint + outclass > 140 && classpoint * 7 + outclass * 3 >= 800)return true;
	else return false;
}
...
{
	string name[1000];
	int classpoint[1000],outclass[1000], n;
	cin >> n;
	for (int i = 0; i < n; i++)cin >> name[i] >> classpoint[i] >> outclass[i];

	for (int i = 0; i < n; i++)
	{
		if (is_excellent(classpoint[i], outclass[i]))cout << "Excellent" << endl;
